...
public class ParseHtmlUtil {
    ...
    public static String[] parseDictWordContent(String htmlTxt) {
        ...
        Elements elementsPhonetic = doc.getElementsByClass("phonetic");
        // System.out.println("elementsPhonetic：" + elementsPhonetic.text());
        String phoneticDJ = "";// 英音
        String phoneticKK = "";// 美音
        if (elementsPhonetic != null && elementsPhonetic.size() > 0) {
            Elements sElementBdos = elementsPhonetic.get(0).getElementsByTag("bdo");
            if (sElementBdos != null) {
                if (sElementBdos.size() > 0) {
                    phoneticDJ = sElementBdos.get(0).text().trim().replace("[", "").replace("]", "");
                }
                if (sElementBdos.size() > 1) {
                    phoneticKK = sElementBdos.get(1).text().trim().replace("[", "").replace("]", "");
                }
            }
            // System.out.println("bdo,phoneticDJ="+phoneticDJ+",phoneticKK="+phoneticKK);
        }
        ...
        String dictBasicMeanings = "";
        if (elementsDictBasic != null && elementsDictBasic.size() > 0) {
            Elements sElementLi = elementsDictBasic.get(0).getElementsByTag("li");
            for (int i = 0; i < sElementLi.size(); i++) {
                String liText = sElementLi.get(i).text().trim();
                if (!"".equals(liText)) {
                    if ("".equals(dictBasicMeanings)) {
                        dictBasicMeanings = liText;
                    } else {
                        dictBasicMeanings += "\n" + liText;
                    }
                    // System.out.println("li,text：" + liText);
                }
            }
        }
        // System.out.println("dictBasicMeanings：\n" + dictBasicMeanings);

        Element elementsDictChart = doc.getElementById("dict-chart-basic");// 单词me没有本属性
        String chartWordMeaning = "";
        ...
        if (elementsDictChart != null && elementsDictChart.hasAttr("data")) {
            ...
            chartWordMeaning = decodeDictWord(chartData);
            pickWordMeaning = pickDictWordMeaning(chartWordMeaning);
        }

        // String [] wordContent = {elementsKeyword.text(),wordShape.replaceAll(" ",
        // "/"),elementsDictBasic.text(),pickWordMeaning};
        String[] wordContent = { elementsKeyword.text(), wordShape.replaceAll(" ", "/"), dictBasicMeanings,
                pickWordMeaning, phoneticDJ, phoneticKK };
        return wordContent;
    ...
    // dict.cn,解码词义及对应的百分比,词义个数不大于4,且获取累计占90%的词义，舍弃剩余的10%
    // "%7B%221%22%3A%7B%22percent%22%3A37%2C%22sense%22%3A%22%5Cu810f%5Cu7684%22%7D%2C%222%22%3A%7B%22percent%22%3A27%2C%22sense%22%3A%22%5Cu80ae%5Cu810f%5Cu7684%22%7D%2C%223%22%3A%7B%22percent%22%3A15%2C%22sense%22%3A%22%5Cu5f04%5Cu810f%22%7D%2C%224%22%3A%7B%22percent%22%3A8%2C%22sense%22%3A%22%5Cu5351%5Cu9119%5Cu7684%22%7D%2C%225%22%3A%7B%22percent%22%3A7%2C%22sense%22%3A%22%5Cu4e0b%5Cu6d41%5Cu7684%22%7D%2C%226%22%3A%7B%22percent%22%3A3%2C%22sense%22%3A%22%5Cu73b7%5Cu6c61%22%7D%2C%227%22%3A%7B%22percent%22%3A2%2C%22sense%22%3A%22%5Cu4e0d%5Cu6b63%5Cu5f53%5Cu7684%22%7D%2C%228%22%3A%7B%22percent%22%3A1%2C%22sense%22%3A%22%5Cu8150%5Cu8d25%5Cu7684%22%7D%7D";
    // the
    ...
    public static String pickDictWordMeaning(String wordString) {
        String wordMeanings = "";
        int totalPercent = 0;
        // List<PercetWord> percetWordList = FastJsonUtil.json2list(wordString,
        // PercetWord.class);
    ...
    public static String parseDictHubMain(String htmlTxt) {
        // Document doc = Jsoup.parse(htmlTxt);
        Document doc = Jsoup.parse(htmlTxt, "UTF-8");
        // Elements elements = doc.getElementsByClass("hub-main");//change-clearfix
        // //in-base
        ...
        Iterator<Element> it = elementsGroups.iterator();
        while (it.hasNext()) {
