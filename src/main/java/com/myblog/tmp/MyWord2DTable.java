...
public class MyWord2DTable {
    ...
    // CATTI(China Accreditation Test for Translators and
    // Interpreters/翻译专业资格（水平）考试)
    // IELTS(雅思)
    // TOEFL(托福)
    // SAT(美国学术能力评估测试/美国高考)
    // GRE(美国研究生入学考试)
    ...
    static String[][] fileLevel = { { "小学", "/word_primary_school.txt", }, // 1353
            { "初中", "/word_junior_school.txt" }, // 2171
            { "高中", "/word_senior_school.txt", }, // 2361
            { "大学", "/word_university.txt", }, // 1864
            { "大学教纲", "/大学英语课程教学要求.txt" }, //
            { "新概念", "/word_new_conception.txt" }, { "四级", "/word_CET4.txt" }, { "六级", "/word_CET6.txt" },
            { "考研", "/word_考 研.txt" }, { "专四", "/word_TEM4.txt" }, { "专八", "/word_TEM8.txt" },
            { "BEC", "/word_BEC.txt" }, { "CATTI", "/word_CATTI.txt" }, { "IELTS", "/word_IELTS.txt" },
            { "TOEFL", "/word_TOEFL.txt" }, { "SAT", "/word_SAT.txt" }, { "GRE", "/word_GRE.txt" },
            { "GMAT", "/word_GMAT.txt" }, { "MBA", "/word_MBA.txt" }, { "TOEIC", "/word_TOEIC.txt" }, };
    ...
    public static Map<String, Map<String, String>> getWordList(String[]... files) {
        ...
        for (String[] file : files) {
            ...
            if (file[1].startsWith("/")) {
                url = MyWord2DTable.class.getResource("/resources" + file[1]);
            } else {
                try {
    ...
    public static void save(Map<String, Map<String, String>> mapResult, String path) {
        ...
        try {
            URL url = MyWord2DTable.class.getResource("/resources" + file);
            if (url == null) {
