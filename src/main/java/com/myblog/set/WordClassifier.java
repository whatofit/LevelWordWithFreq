...
public class WordClassifier {
    ...
    public static void classify(Set<Word> words){
        ...
        words.forEach(word -> {
            ...
            showStatus(data, i.incrementAndGet(), words.size(), word.getSpelling());
            String html = getContent(word.getSpelling());
            //LOGGER.debug("获取到的HTML：" +html);
            while(html.contains("非常抱歉，来自您ip的请求异常频繁")){
                //使用新的IP地址
                DynamicIp.toNewIp();
                html = getContent(word.getSpelling());
            }
            if(StringUtils.isNotBlank(html)) {
                parse(word.getSpelling(), html, data);
                if(!NOT_FOUND_WORDS.contains(word.getSpelling())) {
                    ORIGIN_HTML.add(word.getSpelling() + "杨尚川" + html);
                }
            }else{
                NOT_FOUND_WORDS.add(word.getSpelling());
            }
            ...
            for(Path path : fs.getRootDirectories()){
                ...
                Files.walkFileTree(path, new SimpleFileVisitor<Path>(){
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                        ...
                        // 拷贝到本地文件系统
                        Path temp = Paths.get("./src/target/origin-html-temp.txt");
    ...
    public static void save(Map<String, List<String>> data){
            ...
            data.keySet().forEach(key -> {
                try {
                    String file = "./src/resources/my_word_" + key + ".txt";
                    Path path = Paths.get(file);
                    LOGGER.error("保存词典文件：" + file);
                	List<String> existWords = new ArrayList<String>();
                	if (path.toFile().exists()) {
        				existWords = Files.readAllLines(path);
                	}
        ...
        try {
            ...
            if(!ORIGIN_HTML.isEmpty()) {
                String path = "src/resources/my_origin_html_" + System.currentTimeMillis() + ".txt";
                LOGGER.error("保存词典文件：" + path);
